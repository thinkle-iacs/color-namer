<script lang="ts">
  ...
  const mockGame: GameDoc = {
    ...
    players: {
      ...
      'player-2': { name: 'Bob', score: 31, avatarColor: 'hsl(138, 70%, 50%)', order: 1 },
      'player-3': { name: 'Carol', score: 58, avatarColor: 'hsl(275, 70%, 50%)', order: 2 },
    },
  ...
  let amPicker = $derived(viewAs === mockGame.playerOrder[mockGame.pickerIndex]);
...
<div class="test-page">
  <div class="test-controls">
    ...
    <div class="control-row">
      <span>View as:</span>
      <button class:active={pov === 'guesser'} onclick={() => pov = 'guesser'}>Guesser (Bob)</button>
      <button class:active={pov === 'picker'} onclick={() => pov = 'picker'}>Picker (Alice)</button>
    </div>
    <div class="links">
...
<style>
  ...
  .control-row { display: flex; align-items: center; gap: 0.4rem; font-size: 0.85rem; }
  ...
  .control-row button.active { background: #333; color: #fff; border-color: #666; }
  .links { font-size: 0.8rem; color: #666; }
  .links a { color: #6af; }
  .advanced-msg { font-size: 0.8rem; color: #6f9; }
</style>
